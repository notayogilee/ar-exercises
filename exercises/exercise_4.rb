...
store = Store.create(name: "Surry", annual_revenue: 224000, mens_apparel: false, womens_apparel: true)
...
@womens_stores_million = Store.where(womens_apparel: true).where("annual_revenue < ?", 1000000)

@womens_stores_million.each {|store| puts store.name}
